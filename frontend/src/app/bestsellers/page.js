...
export default function BestSellersPage() {
  ...
  // Filtreleme State'leri (Yüksek fiyatlı çok satan ürünlerin gizlenmemesi için maxPrice 100.000 yapıldı)
  ...
  // ÖNEMLİ: Sayfa ilk açıldığında sıralama varsayılan olarak "bestseller" başlar
  ...
  // 1. Backend'den Filtrelenmiş ve Çok Satanlara Göre Sıralanmış Ürünleri Çekme
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Sıralama parametresini ekliyoruz
        if (sortBy) {
        ...
        setError('Çok satan ürünler yüklenirken bir sorun oluştu.');
        setLoading(false);
      }
    };

    fetchProducts();
  }, [searchTerm, selectedCategory, selectedBrands, selectedColor, maxPrice, sortBy]);
        ...
        <main className="mx-auto max-w-7xl px-4 sm:px-6 py-6 sm:py-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-200 pb-4 mb-6 gap-4">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-base sm:text-lg font-bold tracking-tight text-slate-900 uppercase">Çok Satanlar</h1>
                <p className="text-xs sm:text-sm text-slate-500 mt-0.5">En çok tercih edilen {totalProductsCount} ürün listeleniyor</p>
              </div>
            ...
            <div className="flex items-center gap-3 justify-between md:justify-end">
              ...
              >
                <option value="bestseller">Çok Satanlar (Önerilen)</option>
            ...
            <div className="w-full lg:col-span-4">
              ...
              {!loading && !error && products.length === 0 && (
                <div className="text-center py-24 bg-slate-50 rounded-2xl border border-slate-100 text-slate-500 text-sm sm:text-base font-medium">
                  Çok satan kriterlerine uygun ürün bulunamadı.
                </div>
              )}
