...
export default function AllProductsPage() {
  ...
  // URL'deki keyword değiştiğinde searchTerm state'ini güncelle
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    fetchProducts();
  }, [searchTerm, selectedCategory, selectedBrands, maxPrice, sortBy]);
  ...
  // Filtre Bileşeni (Hem Mobil Modal hem Masaüstü Sidebar için ortak)
  const FilterContent = () => (
        ...
        <main className="mx-auto max-w-7xl px-4 sm:px-6 py-6 sm:py-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-200 pb-4 mb-6 gap-4">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-base sm:text-lg font-bold tracking-tight text-slate-900 uppercase">Tüm Ürünler</h1>
                <p className="text-xs sm:text-sm text-slate-500 mt-0.5">{totalProductsCount} ürün listeleniyor</p>
              </div>
          ...
          {/* Ana İçerik Grid: Genişletilmiş Sol Filtre + Sağ Ürün Listesi */}
